...
(function () {
    ...
    app.controller('PVController', function ($scope, $http, $timeout, $window, $filter, $location, $rootScope) {
        ...
        $scope.ChangeModifier = function () {
            ...
            if (!$scope.cat.Modifier) {
                $scope.Characteristics = null;
                return;
            }

            // Fire multiple calls in parallel
            let requests = {
                unspsc: $http.get('/GeneralSettings/GetUnspsc', {
                    params: { Noun: $scope.cat.Noun, Modifier: $scope.cat.Modifier }
                }),
                hsn: $http.get('/GeneralSettings/GetHsn', {
                    params: { Noun: $scope.cat.Noun, Modifier: $scope.cat.Modifier }
                }),
                uom: $http.get('/Dictionary/getuomlist1', {
                    params: { Noun: $scope.cat.Noun, Modifier: $scope.cat.Modifier }
                }),
                formatted: $http.get('/Dictionary/GetForamted', {
                    params: { Noun: $scope.cat.Noun, Modifier: $scope.cat.Modifier }
                }),
                nounMod: $http.get('/Dictionary/GetNounModifier2', {
                    params: { Noun: $scope.cat.Noun, Modifier: $scope.cat.Modifier }
                }),
                units: $http.get('/Catalogue/GetUnits')
            };

            $q.all(requests).then(function (results) {
                // UNSPSC
                let unspscRes = results.unspsc.data;
                if (unspscRes && unspscRes.length > 0) {
                    $scope.Commodities = unspscRes;
                    $scope.cat.Unspsc = unspscRes[0].Commodity || unspscRes[0].Class;
                } else {
                    $scope.Commodities = null;
                }

                // HSN
                let hsnRes = results.hsn.data;
                if (hsnRes.HSNID) {
                    $scope.HSNID = hsnRes.HSNID;
                    $scope.Desc = hsnRes.Desc;
                    $scope.HSNShow = true;
                } else {
                    $scope.HSNID = null;
                    $scope.Desc = null;
                ...
                // UOM
                let uomRes = results.uom.data;
                if (uomRes.length > 0) {
                    $scope.uomList1 = uomRes;
                } else {
                    // fallback call
                    $http.get('/Catalogue/getuomlist').then(r => {
                        $scope.uomList1 = r.data;
                    });
                }

                // Formatted
                $scope.Type = results.formatted.data;
                $scope.ven = ($scope.Type === "OPM" || $scope.Type === "OEM");

                // Noun Modifier 2
                let nmRes = results.nounMod.data;
                if (nmRes) {
                    $scope.NM1 = nmRes.One_NounModifier;
                    $scope.Characteristics = nmRes.ALL_NM_Attributes;
                } else {
                    $scope.Characteristics = null;
                }

                // Units
                $scope.UOMs = results.units.data;
            }, function (err) {
                console.error("ChangeModifier failed", err);
            });
        };
